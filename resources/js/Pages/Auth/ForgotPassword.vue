    ...
    <main class="auth-shell">
        <form class="card w-full max-w-md space-y-5 p-4 sm:p-6" @submit.prevent="submit">
            <div>
                ...
                <h1 class="text-2xl font-bold text-white">{{ t('Reset your password') }}</h1>
                <p class="mt-1 text-sm text-slate-400">{{ t('Enter your account email and VolumeVault will send a reset link if mail is configured.') }}</p>
            </div>

            <div v-if="!mailResetEnabled" class="rounded-2xl border border-amber-400/30 bg-amber-400/10 p-3 text-sm text-amber-100">
                {{ t('Password reset by email is not configured. Use the CLI reset command from the container instead.') }}
            </div>
            ...
            <button class="btn-primary w-full" :disabled="form.processing || !mailResetEnabled">{{ t('Send reset link') }}</button>
